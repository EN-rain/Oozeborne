...
func _ready() -> void:
	...
	if death_card_animation_player:
		death_card_animation_player.process_mode = Node.PROCESS_MODE_WHEN_PAUSED
	if death_card:
		death_card.process_mode = Node.PROCESS_MODE_WHEN_PAUSED
	if death_card_content:
		death_card_content.process_mode = Node.PROCESS_MODE_WHEN_PAUSED
	if restart_button:
...
func show_death_screen(final_score: int, killer_name: String) -> void:
	...
	visible = true
	_show_death_screen_immediately()
	get_tree().paused = true


func _show_death_screen_immediately() -> void:
	if color_rect:
		color_rect.color = Color(1, 1, 1, 0.6)
	if death_card:
		death_card.anchor_top = 0.5
		death_card.anchor_bottom = 0.5
		death_card.modulate = Color(1, 1, 1, 1)
	if death_card_content:
		death_card_content.modulate = Color(1, 1, 1, 1)
...
func _load_death_messages() -> void:
	...
	var funny_suffixes := parsed.get("funny_suffixes", [])
